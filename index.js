...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (personIcons.length > 0) {
    ...
    const fadeCount = Math.round(totalIcons * 0.2); // 32 icons

    // Seeded deterministic shuffle to select stable, scattered icons
    ...
    personIcons.forEach((el, i) => {
      if (fadeIndices.has(i)) {
        // Deterministic thresholds based on index
        const startSeed = i * 1.5;
        const startRand = Math.sin(startSeed) * 10000 - Math.floor(Math.sin(startSeed) * 10000);
        const durationRand = Math.sin(startSeed + 1.2) * 10000 - Math.floor(Math.sin(startSeed + 1.2) * 10000);

        const startThreshold = startRand * 0.55;
        const endThreshold = startThreshold + 0.2 + durationRand * 0.25;

        el.classList.add('fading-active');
        fadingIcons.push({
          element: el,
  ...
  function updateIconOpacity(designScrollY) {
    ...
    // Directly set opacity of the 32 fading icons in JavaScript
    for (let i = 0; i < fadingIcons.length; i++) {
      const icon = fadingIcons[i];
      let iconProgress = 0;
      if (progress < icon.start) {
        ...
        iconProgress = (progress - icon.start) / (icon.end - icon.start);
      }
      const opacity = 1.0 - iconProgress * 0.85; // goes from 1.0 to 0.15
      icon.element.style.opacity = opacity.toFixed(3);
    }
  }

  // Pre-render a high-quality particle glow template once to optimize frame drawing
  const glowCanvas = document.createElement('canvas');
  glowCanvas.width = 60;
  glowCanvas.height = 60;
  const glowCtx = glowCanvas.getContext('2d');
  const grad = glowCtx.createRadialGradient(30, 30, 0, 30, 30, 30);
  grad.addColorStop(0, 'rgba(232, 245, 189, 1)');      // Primary green-yellow color (#e8f5bd)
  grad.addColorStop(0.3, 'rgba(232, 245, 189, 0.5)');
  grad.addColorStop(1, 'rgba(232, 245, 189, 0)');
  glowCtx.fillStyle = grad;
  glowCtx.beginPath();
  glowCtx.arc(30, 30, 30, 0, Math.PI * 2);
  glowCtx.fill();
  ...
  class Particle {
    ...
    draw(ctx) {
      ...
      const size = this.radius * 6; // Drawing radius is radius * 3, so size is radius * 6
      ctx.drawImage(glowCanvas, this.x - size / 2, this.y - size / 2, size, size);
      ctx.restore();
    }
  }
